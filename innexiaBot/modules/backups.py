...
from innexiaBot import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from innexiaBot.__main__ import DATA_IMPORT
...
# from innexiaBot.modules.rules import get_rules
import innexiaBot.modules.sql.rules_sql as rulessql

# from innexiaBot.modules.sql import warns_sql as warnssql
import innexiaBot.modules.sql.blacklist_sql as blacklistsql
from innexiaBot.modules.sql import disable_sql as disabledsql

# from innexiaBot.modules.sql import cust_filters_sql as filtersql
...
def export_data(update, context):
    ...
    with open("PhoenixWarmedbot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("PhoenixWarmedbot{}.backup".format(chat_id), "rb"),
        caption="💾*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `PhoenixWarmedbot-Backup` was specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("PhoenixWarmedbot{}.backup".format(chat_id))  # Cleaning file
...
*Only for group owner:*
 ...
 ❍ /import: Reply to the backup file for the butler / PhoenixWarmed group to import as much as possible, making transfers very easy! \
